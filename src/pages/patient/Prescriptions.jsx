...
function getFechaVencimiento(item) {
  return (
...
export default function PatientPrescriptions() {
  ...
  const loadPrescriptions = useCallback(
    async (targetPage = page, targetPageSize = pageSize) => {
      ...
      setError("");
      try {
        const { data } = await http.get(endpoints.medicalRecords.getPatientPrescriptions, {
        ...
        console.error("Error cargando prescripciones del paciente:", e);
        setRows([]);
        setBackendPages(1);
        setError("No fue posible cargar tus prescripciones. Intenta nuevamente.");
      } finally {
        setLoading(false);
      }
    },
  ...
  return (
    <PageContainer>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-neutral-800">Mis prescripciones</h1>
          <p className="mt-1 text-sm text-neutral-600">
            Revisa tus prescripciones dispensadas y el detalle de cada item.
          </p>
        </div>
      </div>
      ...
      <DataTable
        ...
        emptyMessage={loading ? "Cargando..." : "No tienes prescripciones para mostrar."}
      />
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
              <div className="rounded-lg border border-neutral-200 p-3">
                ...
                <p className="mt-1 text-sm font-semibold text-neutral-800">
                  {selected.id_atencion ?? "—"}
                </p>
              </div>
              <div className="rounded-lg border border-neutral-200 p-3">
                <p className="text-xs uppercase text-neutral-500">Tipo</p>
                <p className="mt-1 text-sm font-semibold text-neutral-800">
                  {mapTipoLabel(selected.tipo)}
                </p>
              </div>
              <div className="rounded-lg border border-neutral-200 p-3">
                <p className="text-xs uppercase text-neutral-500">Total ítems</p>
                <p className="mt-1 text-sm font-semibold text-neutral-800">
                  {Array.isArray(selected.prescripciones_items)
                    ? selected.prescripciones_items.length
                    : 0}
                </p>
              </div>
            </div>
